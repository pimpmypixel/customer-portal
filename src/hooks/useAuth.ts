...
export const useAuth = ({ middleware, redirectIfAuthenticated }: IUseAuth) => {
    ...
    /*const register = async (args: IApiRequest) => {
        ...
        axios
            .post('/register', props)
            .then(() => mutate())
            .catch((error: { response: { status: number; data: { errors: SetStateAction<never[]> } } }) => {
    ...
    const forgotPassword = async (args: IApiRequest) => {
        ...
        axios
            ...
            .then((response: { data: { status: any } }) => setStatus(response.data.status))
            .catch((error: { response: { status: number; data: { errors: SetStateAction<never[]> } } }) => {
                if (error.response.status !== 422) throw error
    ...
    const resetPassword = async (args: IApiRequest) => {
        ...
        axios
            ...
            .then((response: { data: { status: string } }) => router.push('/login?reset=' + btoa(response.data.status)))
            .catch((error: { response: { status: number; data: { errors: SetStateAction<never[]> } } }) => {
                if (error.response.status !== 422) throw error
    ...
    const resendEmailVerification = (args: IApiRequest) => {
        ...
        axios.post('/email/verification-notification').then((response: { data: { status: any } }) => setStatus(response.data.status))
    }*/
